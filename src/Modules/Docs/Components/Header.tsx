...
export const Header = HeaderController.view(({ state }) => {
  return (
    <header
      ...
      classList={{
        "dark:bg-slate-900/95 dark:[@supports(backdrop-filter:blur(0))]:bg-slate-900/75": state.isScrolled,
        "dark:bg-transparent": !state.isScrolled
      }}
    >
      <div class="mr-6 flex lg:hidden">
        <MobileNavigation navigation={config.navigation} />
      </div>
      <div class="relative flex flex-grow basis-0 items-center">
        <Link href={config.landing} aria-label="ValkyrJS home">
          <Logo />
        </Link>
      </div>
      <div class="relative flex basis-0 justify-end gap-6 sm:gap-8 md:flex-grow">
        {/* <ThemeSelector class="relative z-10" /> */}
        <Link href="https://github.com/sadoprotocol" target="_blank" class="group" aria-label="GitHub">
