...
import { MarkdownDocument } from "../Library/Markdoc";

export function ContentView(props: any) {
  const state = getContent(props);
  return (
    <Show when={state()}>
      <div class="min-w-0 max-w-2xl flex-auto px-4 py-8 lg:max-w-none lg:pl-8 lg:pr-0 xl:px-16">
        <article>
          ...
          <Prose>
            <Dynamic component={state().content} />
          </Prose>
        </article>
      ...
      <div class="hidden xl:sticky xl:top-[4.5rem] xl:-mr-6 xl:block xl:h-[calc(100vh-4.5rem)] xl:flex-none xl:overflow-y-auto xl:py-3 xl:pr-6">
...
function getContent(props: any) {
  const [state, setState] = createSignal<MarkdownDocument | undefined>();

  createEffect(() => {
    db.docs.findOne({ path: props.path }).then((doc) => {
      if (doc === undefined) {
        return;
      }
      window.scrollTo(0, 0);
      setState(markdoc.getParsedDocument(doc.body));
    });
  });
